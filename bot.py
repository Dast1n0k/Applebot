    ...
    try:
        if call.message:
            if call.data == '1st':
	            ...
	            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text="Провод для Айфона🔋",
	            reply_markup=None)

            elif call.data == 'opt':
                bot.send_message(call.message.chat.id, '10шт. 1000 грн')
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text="Провода для Айфонов 10 штук🔋(можна заказать в любом количестве!)",
                reply_markup=None)
